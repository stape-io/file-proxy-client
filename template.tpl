...

{
  ...
  "description": "This client allows you proxy any file through Server GTM. This is useful if you need to serve files from your domain.",
  "containerContexts": [
    "SERVER"
  ]
}
...
[
  ...
  },
  {
    ...
    "checkboxText": "Use original Response Headers for the response",
    "simpleValueType": true,
    "defaultValue": true
  },
  {
        ...
        "valueValidators": [
          {
            ...
            "errorMessage": "It must match a valid HTTP Status Code according to RFC 9110."
          }
        ],
        "valueHint": "200"
      },
...
2026-04-20 - Change Notes:
  - Fix: cache writes are now guarded by the useCache flag (previously always written)
  - Fix: responseStatusCode is validated before use; invalid values fall back to origin status
  - Fix: sendResponse validates the status code and falls back to 500 if invalid
  - Fix: setResponseBody is skipped when the response body is undefined
  - Add: isValidStatusCode() helper using makeString + String.match regex
  - UI: updated useOriginHeaders checkbox label for clarity
  - UI: added "Add Header" button label to request/response header tables
